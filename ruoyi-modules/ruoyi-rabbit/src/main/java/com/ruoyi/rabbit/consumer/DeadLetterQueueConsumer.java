...
public class DeadLetterQueueConsumer {
    ...
    //
    @RabbitListener(queues = "articleColumnNoticeDelayedQueue")
    public void receiveArticleColumnNoticeDelayedQueue(Message message, Channel channel,String s)  throws Exception{
        long deliveryTag = message.getMessageProperties().getDeliveryTag();
        try {
            log.info("MQ接收文章栏目通知:{}",s);
            System.out.println("111111111");
            //手动确认消息
            channel.basicAck(deliveryTag, false);
        }catch(Exception e){
            log.error("MQ接收文章栏目通知异常:",e);
